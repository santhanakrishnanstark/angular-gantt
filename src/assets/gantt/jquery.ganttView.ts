...
export const ganttView = function (jQuery, id, ...args) {
  jQuery = jQuery;
  var plugin = jQuery(id);

  // var args = Array.prototype.slice.call(arguments);

  // if (args.length == 1 && typeof args[0] == "object") {
  console.log(args[0]);
  build.call(plugin, args[0], jQuery);
  // }

  // if (args.length == 2 && typeof args[0] == "string") {
  //   handleMethod.call(plugin, args[0], args[1]);
  // }
};

function build(options, jQuery) {
  var els = this;
  var defaults = {
    showWeekends: true,
    cellWidth: 21,
  ...
  function build(jQuery) {
    ...
    els.each(function () {
      ...
      Behavior(container, opts, jQuery).apply();
    });
  }
}
...
var Chart = function (div, opts, jQuery) {
  function render() {
    addVtHeader(div, opts.data, opts.cellHeight, jQuery);
  ...
  function addVtHeader(div, data, cellHeight, jQuery) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      for (var j = 0; j < data[i].series.length; j++) {
        seriesDiv.append(
          jQuery("<div>", { class: "ganttview-vtheader-series-name" }).append(
  ...
  function addBlocks(div, data, cellWidth, start) {
    ...
    for (var i = 0; i < data.length; i++) {
      for (var j = 0; j < data[i].series.length; j++) {
        ...
        block.append(
          jQuery("<div>", { class: "ganttview-block-text" }).text(size)
        );
...
var Behavior = function (div, opts, jQuery) {
  function apply() {
    if (opts.behavior.clickable) {
      bindBlockClick(div, opts.behavior.onClick, jQuery);
    }
  ...
  function bindBlockClick(div, callback, jQuery) {
  ...
  function bindBlockResize(div, cellWidth, startDate, callback, jQuery) {
    jQuery("div.ganttview-block", div).resizable({
      grid: cellWidth,
      handles: "e,w",
      stop: function () {
        var block = jQuery(this);
        updateDataAndPosition(div, block, cellWidth, startDate, jQuery);
        if (callback) {
  ...
  function bindBlockDrag(div, cellWidth, startDate, callback, jQuery) {
  ...
  function updateDataAndPosition(div, block, cellWidth, startDate, jQuery) {
